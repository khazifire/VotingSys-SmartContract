...
class RegisterAccountView(generic.View):
    template_name = 'poll/register_account.html'
    ...
    def post(self, *args, **kwargs):
        if self.request.is_ajax and self.request.method == "POST":
            ...
            voter_fname = self.request.POST.get("voter_fname", None)
            voter_lname = self.request.POST.get("voter_lname", None)
            voter_age = self.request.POST.get("voter_age", None)

            if (len(voter_address)) and (len(voter_fname)) and (len(voter_lname)) and (len(voter_address)) and (len(voter_age)):
